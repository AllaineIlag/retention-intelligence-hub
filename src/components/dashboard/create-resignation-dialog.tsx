...
function CreateResignationDialogContent({ open, setOpen }: { open: boolean, setOpen: (o: boolean) => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!lastWorkingDay) {
            toast.error("Please set the last working day")
            return
        }
        ...
        const res = await createResignation({
            ...
            businessUnit: selectedEmployee.business_unit,
            intermediateSupervisor: selectedEmployee.intermediate_supervisor,
                        ...
                        <div className="space-y-5 animate-in fade-in zoom-in-95 duration-300">
                            <div className="p-4 rounded-xl border border-primary/20 bg-primary/5 flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-muted-foreground truncate">{selectedEmployee.department} · {selectedEmployee.business_unit}</p>
                                </div>
                            </div>
